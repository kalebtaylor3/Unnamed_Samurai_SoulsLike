...
public:
	...
	/** Is the player overlapping a bonfire */
	UPROPERTY(BlueprintReadWrite, Category = "Bonfire")
	bool bCanInteractWithBonfire = false;

	/** Pointer to the active bonfire */
	UPROPERTY(BlueprintReadWrite, Category = "Bonfire")
	ABonfire* ActiveBonfire = nullptr;

	UPROPERTY()
	class ALootDropActor* ActiveLootDrop;

	UPROPERTY(BlueprintReadWrite, Category = "Bonfire")
	class UBonfireMenuWidget* CurrentBonfireMenu;

	UPROPERTY(EditDefaultsOnly)
	TSubclassOf<class ARuneDropActor> RuneDropClass;

	/** Trigger bonfire sit animation */
	UFUNCTION(BlueprintCallable, Category = "Bonfire")
	void PlayBonfireSitAnimation(ABonfire* Bonfire);
	...
	bool bIsInvisibleToEnemies = false;

	void Rest();

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Weapon")
	AHeldWeaponBase* HeldWeaponActor;
	...
	TSubclassOf<UPlayerHUDUI> PlayerHUDWidgetClass;

	UPROPERTY(BlueprintReadOnly, Category = "UI")
	UPlayerHUDUI* PlayerHUDWidget = nullptr;
	...
	UPROPERTY(EditAnywhere, Category = "ALS|Input", BlueprintReadOnly)
	...
	UPROPERTY(BlueprintReadOnly, Category = "ALS|Camera")
	AALSPlayerCameraManager* CameraSystem;
	...
	UFUNCTION()
	void UpdateHUDFromStats(float CurrentHealth, float MaxHealth, float CurrentFP, float MaxFP, float CurrentStamina, float MaxStamina, int32 CurrentLevel, int32 MaxLevel);

	UPROPERTY(BlueprintReadOnly, Category = "Inventory Navigation")
	int32 InventoryNavIndex = 100; // The current slot the cursor is on

	UPROPERTY(BlueprintReadOnly, Category = "Inventory Navigation")
	int32 MaxSlotIndex = 13; // 0-9 Backpack + 100-103 Equipped (mapped to 4 extra UI slots)

	void ConfirmSlot();
	void CancelSelection();

	UFUNCTION(BlueprintCallable)
	void NavigateInventory(int32 Direction);

	UFUNCTION(BlueprintCallable)
	void BonfireMenuNavigate(int32 Direction);

	UFUNCTION(BlueprintCallable)
	void BonfireSkillsNavigate(int32 Direction);

	UFUNCTION(BlueprintCallable)
	void BonfireSkillIncreaseNavigate(int32 Direction);
