...
void AALSBaseCharacter::BeginPlay()
{
	...
	/*UGameplayStatics::DeleteGameInSlot(TEXT("BonfireSlot"), 0);*/
	...
	{
		PlayerHUDWidget = CreateWidget<UPlayerHUDUI>(GetWorld(), PlayerHUDWidgetClass);
		if (PlayerHUDWidget)
		{
	...
	if (!CameraSystem)          // not set yet?  grab it once
	{
		if (APlayerController* PC = Cast<APlayerController>(GetController()))
		{
			CameraSystem = Cast<AALSPlayerCameraManager>(PC->PlayerCameraManager);
		}
	}

	if(!CombatSystem)
		CombatSystem = FindComponentByClass<UCombatComponent>();
	...
	if (!PlayerStats)
	{
		PlayerStats = FindComponentByClass<UPlayerStatsComponent>();
	}
	...
	if (PlayerStats)
	{
		if (UMyGameInstance* MyGI = Cast<UMyGameInstance>(UGameplayStatics::GetGameInstance(this)))
		{
			...
			if (MyGI->bShouldRestoreFromBonfire)
			{
				...
				{
					SetActorLocation(MyGI->SavedBonfireLocation);
					SetActorRotation(MyGI->SavedBonfireRotation);
				}

				PlayerStats->CurrentLevel = MyGI->SavedLevel;
				PlayerStats->VigorLevel = MyGI->SavedVigor;
				PlayerStats->MindLevel = MyGI->SavedMind;
				PlayerStats->EnduranceLevel = MyGI->SavedEndurance;

				PlayerStats->RecalculateDerivedStats();
				PlayerStats->CurrentHealth = PlayerStats->MaxHealth;
				PlayerStats->CurrentFP = PlayerStats->MaxFP;
				PlayerStats->CurrentStamina = PlayerStats->MaxStamina;
				PlayerStats->CurrentRunes = MyGI->bWasKilled ? 0 : MyGI->SavedRunes;
			...
			else
			{
				...
				if (UBonfireSaveGame* SaveData = Cast<UBonfireSaveGame>(UGameplayStatics::LoadGameFromSlot(TEXT("BonfireSlot"), 0)))
				{
					...
					PlayerStats->CurrentLevel = SaveData->PlayerLevel;
					PlayerStats->VigorLevel = SaveData->VigorLevel;
					PlayerStats->MindLevel = SaveData->MindLevel;
					PlayerStats->EnduranceLevel = SaveData->EnduranceLevel;

					PlayerStats->RecalculateDerivedStats();
					PlayerStats->CurrentHealth = PlayerStats->MaxHealth;
					PlayerStats->CurrentFP = PlayerStats->MaxFP;
					PlayerStats->CurrentStamina = PlayerStats->MaxStamina;
					PlayerStats->CurrentRunes = SaveData->CurrentRunes;
	...
	if (CombatSystem && PlayerStats)
	{
		PlayerStats->CombatComponent = CombatSystem;
	}

	if (PlayerStats)
	{
		// Delay the binding by 1 frame
		FTimerHandle DelayHandle;
		GetWorld()->GetTimerManager().SetTimer(DelayHandle, [this]()
			{
				if (PlayerStats)
				{
					PlayerStats->OnStatsChanged.AddDynamic(this, &AALSBaseCharacter::UpdateHUDFromStats);

					// Immediately push current stats to the HUD
					UpdateHUDFromStats(
						PlayerStats->CurrentHealth,
						PlayerStats->MaxHealth,
						PlayerStats->CurrentFP,
						PlayerStats->MaxFP,
...
void AALSBaseCharacter::Tick(float DeltaTime)
{
	...
	if (bSprintHeld && PlayerStats)
	{
		float StaminaDrainRate = 10.f; // per second
		PlayerStats->UseStamina(StaminaDrainRate * DeltaTime);
	}
}
...
void AALSBaseCharacter::RagdollStart()
{
	...
	// 🔒 Hard stop all movement
	GetMesh()->SetAllPhysicsLinearVelocity(FVector::ZeroVector);
	GetMesh()->SetAllPhysicsAngularVelocityInDegrees(FVector::ZeroVector);
	GetMesh()->PutAllRigidBodiesToSleep();

	// Optional: remove collision response so it can't be launched again
...
void AALSBaseCharacter::PlayBonfireSitAnimation(ABonfire* Bonfire)
{
	if (!Bonfire || !SitMontage || !SitLoopMontage) return;

	ActiveBonfire = Bonfire;

	// Play the initial sit-down montage
	PlayAnimMontage(SitMontage);

	// After SitMontage ends, play the looping idle sit animation
	const float SitDuration = SitMontage->GetPlayLength();
...
void AALSBaseCharacter::UpdateHUDFromStats(float CurrentHealth, float MaxHealth, float CurrentFP, float MaxFP, float CurrentStamina, float MaxStamina, int32 CurrentLevel, int32 MaxLevel)
{
	if (PlayerHUDWidget)
	{
		PlayerHUDWidget->SetStats(CurrentHealth, MaxHealth, CurrentFP, MaxFP, CurrentStamina, MaxStamina, CurrentLevel, MaxLevel);
	}
}
...
void AALSBaseCharacter::Rest()
{
	Inventory->CurrentHPPotions = Inventory->MaxHPPotions;
	Inventory->CurrentFPPotions = Inventory->MaxFPPotions;
	Inventory->UpdatePotionHUD();
	PlayerStats->RestoreFP(PlayerStats->MaxFP);
...
void AALSBaseCharacter::JumpAction_Implementation(bool bValue)
{
	...
	if (CombatSystem->bIsAttacking)
		return;

	if (Inventory->bHealing)
		return;
...
void AALSBaseCharacter::CrouchAction_Implementation(bool bValue)
{
	if (bValue)
	{
		if (MovementState == EALSMovementState::Grounded)
...
void AALSBaseCharacter::SprintAction_Implementation(bool bValue)
{
	...
	if (bValue)
	{
		if (PlayerStats->CurrentStamina > 0)
		{
			...
			GetWorld()->GetTimerManager().ClearTimer(PlayerStats->StaminaRegenHandle);
			PlayerStats->bCanRegenStamina = false;
		}
		else
		{
			PlayerStats->NotifyStaminaExhausted();
...
void AALSBaseCharacter::RollAction_Implementation()
{
	...
	// Step 2: Rotate to that direction
	FRotator DesiredRotation = RollDirection.Rotation();
	SetActorRotation(DesiredRotation);

	if (GetOverlayState() != EALSOverlayState::Default && GetOverlayState() != EALSOverlayState::Bow)
	{
		// Step 3 (optional): Launch in that direction
		LaunchCharacter(RollDirection * 1100, true, true); // tweak strength

		// Step 4: Play the roll animation
		Replicated_PlayMontage(GetRollAnimation(), 1.6f);
	}
	else
	{
		// Step 3 (optional): Launch in that direction
		LaunchCharacter(RollDirection * 1000, true, true); // tweak strength

		// Step 4: Play the roll animation
		Replicated_PlayMontage(GetRollAnimation(), 1.8f);
	}
	...
	if (PlayerStats)
	{
		PlayerStats->UseStamina(25.f);

		PlayerStats->bIsInvincible = true;

		// 26 frames at 60fps = ~0.433 seconds
		GetWorld()->GetTimerManager().SetTimer(
			PlayerStats->IFrameResetHandle,
			PlayerStats,
			&UPlayerStatsComponent::ResetInvincibility,
			0.433f,
			false
		);
	}
...
void AALSBaseCharacter::BonfireMenuNavigate(int32 Direction)
{
	if (UBonfireMenuWidget* Menu = Cast<UBonfireMenuWidget>(CurrentBonfireMenu))
	{
		Menu->Navigate(Direction);
	}
}

void AALSBaseCharacter::BonfireSkillsNavigate(int32 Direction)
{
	if (UBonfireMenuWidget* Menu = Cast<UBonfireMenuWidget>(CurrentBonfireMenu))
	{
		Menu->NavigateSkills(Direction);
	}
}

void AALSBaseCharacter::BonfireSkillIncreaseNavigate(int32 Direction)
{
	if (UBonfireMenuWidget* Menu = Cast<UBonfireMenuWidget>(CurrentBonfireMenu))
	{
		Menu->UpdateSkillLevel(Direction);
	}
}

void AALSBaseCharacter::NavigateInventory(int32 Direction)
{
	if (!PlayerHUDWidget || !Inventory || !Inventory->bIsInventoryOpen) return;

	const int32 EquippedStart = 100;
	const int32 EquippedEnd = 103;
	const int32 BackpackStart = 0;
	...
	{
	case 0: // Up
		if (InventoryNavIndex >= 5 && InventoryNavIndex <= 9)
		{
			InventoryNavIndex -= 5;
		}
		else if (InventoryNavIndex <= 4)
		{
			int32 Column = InventoryNavIndex % 5;
			if (Column < 4)
			{
				InventoryNavIndex = 100 + Column;
			}
		}
		break;
	case 1: // Down
		if (InventoryNavIndex >= 100 && InventoryNavIndex <= 103)
		{
			int32 EquippedCol = InventoryNavIndex - 100;
			InventoryNavIndex = EquippedCol;
		}
		else if (InventoryNavIndex >= 0 && InventoryNavIndex <= 4)
		{
			InventoryNavIndex += 5;
		}
		break;
	case 2: // Left
		if (InventoryNavIndex > BackpackStart && InventoryNavIndex <= BackpackEnd)
		{
			InventoryNavIndex--;
		}
		else if (InventoryNavIndex > EquippedStart && InventoryNavIndex <= EquippedEnd)
		{
			InventoryNavIndex--;
		}
		break;
	case 3: // Right
		if (InventoryNavIndex < BackpackEnd)
		{
			InventoryNavIndex++;
		}
		else if (InventoryNavIndex >= EquippedStart && InventoryNavIndex < EquippedEnd)
	...
	if (PrevIndex != InventoryNavIndex)
	{
		if (PlayerHUDWidget)
		{
			// Clear just the previous navigation highlight, *not* the selected swap slot
			if (PrevIndex != Inventory->SelectedSlotIndex)
			{
				PlayerHUDWidget->SetSlotToDefaultColor(PrevIndex);
			}

			// Keep the selected swap slot yellow
			PlayerHUDWidget->HighlightSlot(InventoryNavIndex);
		}
	}
}
...
void AALSBaseCharacter::ConfirmSlot()
{
	if (!Inventory || !PlayerHUDWidget) return;

	// Use Hovered index if inventory is being controlled via mouse
	int32 IndexToConfirm = InventoryNavIndex;

	// Mouse takes priority if hovering
	if (PlayerHUDWidget->IsMouseHoveringSlot())
	{
		IndexToConfirm = PlayerHUDWidget->GetHoveredSlotIndex();
	}

	Inventory->SelectSlot(IndexToConfirm);
}

void AALSBaseCharacter::CancelSelection()
{
	Inventory->ClearSelection();
}

void AALSBaseCharacter::ToggleInventory()
{
